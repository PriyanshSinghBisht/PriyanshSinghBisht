# 👋 Hi, I'm Priyansh Singh Bisht

**Fullstack Web Developer**  
React.js • Next.js • Node.js • Express • MERN Stack

---

### 🚀 About Me

I'm a passionate fullstack developer specializing in building robust, scalable, and modern web applications using the MERN stack. I love turning ideas into reality through code and am always eager to learn new technologies and best practices.

---

### 🛠️ Tech Stack

- **Frontend:** React.js, Next.js, Redux, React Hook Form, HTML5, CSS3, JavaScript (ES6+)
- **Backend:** Node.js, Express.js
- **Database:** MongoDB, Mariadb, SQL/NO SQL, Prisma, ORM
- **Programming Languages:** C ( fundamental ), C++ ( fundamental ), Java ( fundamental ), Python (basics), Javascript ( advance ), Typescript (advance), Rust (very basic), bash ( advance )
- **Others:** Responsive Design, REST APIs, Git & GitHub

---

### 🌟 Featured Projects

- [messenger-clone](https://github.com/PriyanshSinghBisht/messenger-clone): A real-time chat application inspired by Facebook Messenger, built with the MERN stack.
- [discord-clone](https://github.com/PriyanshSinghBisht/discord-clone): A Discord-inspired chat platform with channels, direct messaging, and more.

---

### 📫 Connect with Me

- [LinkedIn](https://www.linkedin.com/in/priyansh-bisht-a45751235)
- [Portfolio](https://pr1yansh.vercel.app)
- [Leetcode](https://leetcode.com/u/Priyansh2552/)
- **Email:** Priyanshbisht5@gmail.com

---

<!--
Want to add a fun fact, mission statement, or more personal info? Let me know!
-->
